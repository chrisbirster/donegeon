...
const QUICK_ADD_TOKEN_PATTERN =
  /(\{[^{}]+\}|#[A-Za-z][A-Za-z0-9_-]*|@[A-Za-z][A-Za-z0-9_-]*|\+[A-Za-z][A-Za-z0-9_-]*|\bp[1-4]\b|\bevery\s+[A-Za-z0-9 ]+\b|\bin\s+\d+\s+(?:day|days|week|weeks|month|months)\b|\btomorrow\b)/gi;
...
function boardChoicesFromProjects(projects: Project[], activeBoardID: string): BoardChoice[] {
  ...
  if (!byBoardID.has(normalizedActive)) {
    byBoardID.set(normalizedActive, {
      ...
      isTeamBoard: normalizedActive !== DEFAULT_BOARD,
    });
  }
...
export default function BoardRoute() {
  ...
  async function listAllTasks(limit = 100): Promise<Task[]> {
    ...
    for (let page = 0; page < 100; page += 1) {
      const response = await taskApi.list({ limit, cursor });
  ...
  async function loadBoard(options: { syncTasks?: boolean; boardID?: string } = {}) {
    ...
    try {
      ...
      if (Object.keys(response.stacks ?? {}).length === 0) {
        ...
        response = await boardApi.getState(boardID);
      }
      setState(response);
      let syncError = "";
  ...
  function onComposerInput(value: string) {
    ...
    composerParseTimer = window.setTimeout(async () => {
      ...
      try {
        const parsed = await parseApi.quickAdd(ensureBoardProjectToken(trimmed, activeBoardProjectID()));
  ...
  async function createTaskStack() {
    ...
    try {
      const created = await taskApi.quickAdd(normalizedQuickAdd);
      if (!matchesBoardProject(created.task.projectId, boardID)) {
        await taskApi.update(created.task.id, { projectId: boardProjectID });
      }
  ...
  async function saveDetail() {
    ...
    try {
      ...
      if ((recurrenceEnabled || deadlineEnabled) && rawTitle !== "") {
        const parsed = await parseApi.quickAdd(rawTitle);
      ...
      if (!taskID) {
        const created = await taskApi.create(normalizedContent);
        taskID = created.id;
