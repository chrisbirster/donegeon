...
import { APP_URL, LOGIN_URL, PLAN_LINKS, defaultPublicConfig, waitlistHref } from "../lib/site";
...
export default function MarketingLayout(props: ParentProps) {
  ...
  const [publicConfig, setPublicConfig] = createSignal(defaultPublicConfig());
  ...
  const signInHref = () => (publicConfig().openBeta ? LOGIN_URL : waitlistHref({ source: "marketing-nav" }));
  const primaryCtaHref = () =>
    publicConfig().openBeta ? PLAN_LINKS.personal : waitlistHref({ source: "marketing-footer", plan: "personal" });
  const secondaryCtaHref = () =>
    publicConfig().openBeta ? PLAN_LINKS.proTrial : waitlistHref({ source: "marketing-footer", plan: "pro_trial" });
        ...
        <Show when={configChecked() && !publicConfig().openBeta}>
          <div class="border-b border-[rgba(255,255,255,0.08)] bg-[rgba(255,139,80,0.14)]">
            <div class="mx-auto flex max-w-6xl flex-col gap-2 px-6 py-3 text-sm text-[#ffe0c8] md:flex-row md:items-center md:justify-between md:px-10">
              <p>Open beta starts {publicConfig().openBetaStartsLabel}. Click here to sign up for the waitlist.</p>
              <a
            ...
            <div class="flex items-center gap-4">
              <A href="/" class="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p class="text-xs uppercase tracking-[0.18em] text-[var(--text-muted)]">Strategic task software</p>
                </div>
              </A>
            </div>
        ...
        <section class="mx-auto max-w-6xl px-6 pb-8 md:px-10 md:pb-12">
          <div class="overflow-hidden rounded-[2rem] border border-[var(--border-strong)] bg-[linear-gradient(135deg,rgba(255,139,80,0.12),rgba(103,187,255,0.07)_55%,rgba(138,228,163,0.08))] p-8 shadow-[0_30px_60px_rgba(0,0,0,0.28)]">
            <div class="grid gap-6 md:grid-cols-[minmax(0,1fr)_auto] md:items-end">
              <div>
                <p class="section-label">Ready to ship a better workflow</p>
                <h2 class="font-display text-3xl font-semibold text-white md:text-4xl">
                  Move from backlog chaos to a board your team actually wants to use.
                </h2>
                <p class="mt-3 max-w-2xl text-base text-[var(--text-soft)] md:text-lg">
                  Start on Personal, activate the Pro trial when you need team boards, or talk to us about enterprise rollout.
                </p>
              </div>
          ...
          <div class="mx-auto grid max-w-6xl gap-8 px-6 py-10 text-sm md:grid-cols-4 md:px-10">
            <div>
              ...
              <p class="mt-3 max-w-xs text-[var(--text-muted)]">
                Task management for teams that want structured operations and a board with actual stakes.
              </p>
            </div>
            ...
            <div>
              ...
              <div class="mt-3 flex flex-col gap-2 text-[var(--text-muted)]">
                ...
                <span>Built with Go, SolidJS, and a board-first product model.</span>
              </div>
            </div>
          </div>
        </footer>
